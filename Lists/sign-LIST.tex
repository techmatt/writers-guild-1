...
\NEW{Sign}{\sBatterySource}{%%3 sources in game, each has 3 batteries, for a total of 9 available in game.
  ...
  \s\MYloc	{200-002}
  \s\MYtext	{This chemical stock room has sulfuric acid in it. You can build a battery here. To do so, take 2 minutes and consume 1 Copper (item number: \iCopper{\MYnumber{}}), and 1 Zinc (item number: \iZinc{\MYnumber{}}) (tear them up), then take \iBattery{} (item number:\iBattery{\MYnumber{}}). If this location has no batteries available, all of the sulfuric acid has been consumed and you must find another source.}
...
\NEW{Sign}{\sWaterPond}{%%Water elemental residue source
  ...
  {\bf If you draw water from the pond for any reason, the item in which you put the water acquires an $\alpha$ score of ``2''.} (Write ``$\alpha$ = 2'' on the item.)}
  \s\MYitems {}
}
...
\NEW{Sign}{\sLocationCoverSheet}{
  \s\MYname	{An Unsealed Possible Location of Conduit Contact}
  \s\MYloc	{}%%TBD
