...
\NEW{Sign}{\sConduitOne}{
  \s\MYname	{A Possible Location of Closest Contact}
  \s\MYloc	{}%%TBD
  \s\MYtext	{To determine if this is the current location of closest contact, search the chart below for the number associated with the current time. The number associated with the current time is the {\bf only} active number. Numbers associated with previous or future time slots are not active \textbf{and cannot be checked}.
...
%%GD Lab Signs 
  ...
  \s\MYloc	{}%%TBD
  \s\MYtext	{This is a laboratory filled with a wide array of complex and only semi-portable machinery and equipment. It has evidently been setup in considerable haste. You cannot interact with this sign unless you know otherwise.
  } 
}
...
\NEW{Sign}{\sGDLabChair}{
  ...
  \s\MYloc	{}%%TBD
  \s\MYtext	{A metal chair. This chair has numerous wires running from it to near-by monitors. There also wrist and ankle cuffs for securing the test subject. 
  
  The chair crucial for certain experiments. It is bolted to the floor and cannot be moved or destroyed.
  
  \emph{If a player is instructed to sit in the chair, the should role play accordingly.}
  } 
}
  
\NEW{Sign}{\sGDLabBench}{
  \s\MYname	{A Wooden Lab Bench}
  \s\MYloc	{-002}
  \s\MYtext	{This bench is littered with items relevant to doing scientific research.
  } 
  \s\MYitems{\iPhaseTransducer{}}
}
...
\NEW{Sign}{\sAziSciLabBench}{
  ...
  {\bf You cannot interact with this sign unless you know otherwise.}
  } 
  \s\MYitems{\iTransmogrifier{} {\multi{10}{\iElementium{}}}}%%Elementium lives in the ``A'' Packet.
}
...
\NEW{Sign}{\sMSLab}{
  ...
  \s\MYloc	{}%%TBD
  \s\MYtext	{Throughout this room are hastily setup computers and mechanical apparatus in various stages of assembly. You cannot interact with this sign unless you know otherwise. 
  } 
  }
...
\NEW{Sign}{\sJanitorCloset}{
  ...
  \s\MYloc	{-013}
  \s\MYtext	{There is a Caretaker's cottage tucked away at the edge of the gardens. An estate this big ought to have a caretaker to maintain the grounds, but it is clear that no one has lived here for years. Luckily much of the caretaker equipment is still here.
...
\NEW{Sign}{\sWaterPond}{%%Water elemental residue source
  ...
  \s\MYloc	{-032}
  \s\MYtext	{There is a pond of water here. You can't tell how deep it is in the middle, but here at the edge, it is shallow enough. The pond can be used as a source of water.
...
\NEW{Sign}{\sLavaPool}{
  ...
  This lava pool can be used as a heat source, {\bf but if you remain within 1 ZoC of the lava for more than 1 minute, you suffer from heat fatigue} (you can only walk heel to toe for the next 3 minutes).
  }
  \s\MYitems {}
}
